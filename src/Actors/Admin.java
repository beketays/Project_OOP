...
//	}
	...
	public void manageUsers(User u, Command c) throws IOException {
		...
		if(c == Command.REMOVE && (u instanceof Manager))manageManager((Manager)u, Command.ADD_MANAGER);
				...
				if(choice==1){
					...
					if(choice == 2) {
						for(Message m : this.getMessages()) {
							if(m.type == MessageType.RECEIVED) System.out.println(m);
						}
					}
					if(choice == 3) {
						for(Message m : this.getMessages()) {
					...
					manageUsers: while(true) {
						...
						if(choice == 1) {
							addUser: while(true) {
								...
								if(choice == 1) {
									...
									if(degree == Degree.BACHELOR) {
										...
										Researcher supervisor = db.researchers.get(index-1);
							...
							removeUser: while(true) {
								System.out.println(Translator.translate(" Enter id of the user to delete "));
								String ids = in.next();
								User u = null;
								for(User i : db.users) {
									if(i.getId().equals(ids))u = i;
								}
								...
								if(choice == 1) {
									...
									if(choice ==1) {//assign stud
										if(!db.students.isEmpty()) {
											...
											Researcher r = new Researcher(s.getKey(), s.getName(), s.getSurname(), s.getId(), s);
											manageUsers(r,Command.ADD_RES);
										}
